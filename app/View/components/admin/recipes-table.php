    ...
    <?php foreach ($data as $row): ?>
      <tr>
        <td style="display:flex; align-items:center;">
          ...
          <img src="<?= BASE_APP_DIR ?>/public/images/recipesImages/<?= htmlspecialchars($row->image_url) ?>" alt=""
            style="width: 50px; height: 50px; border-radius: 50%;">
          <p style="padding-left:10px"><?= htmlspecialchars($row->name) ?></p>
        </td>
...
<div class="modal fade" id="recipeDetailsModal" tabindex="-1" role="dialog" aria-labelledby="recipeDetailsModalLabel"
  aria-hidden="true">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body px-4">
        <form action="" method="post" id="recipe-form-data" enctype="multipart/form-data">
          ...
          <div class="form-group">
            <input type="number" name="edit_calories" id="edit_calories" class="form-control" placeholder="Calories"
              required>
          </div>
          <div class="form-group">
          ...
          <div class="form-group">
            <input type="hidden" name="edit_creator" class="form-control" placeholder="Creator ID" required
              value="<?= $_SESSION['id'] ?>">
          </div>
          <div class="form-group">
            <label for="edit_imageUpload">Choose recipe image</label>
            <input type="file" id="edit_imageUpload" name="edit_imageUpload" accept=".png, .jpg, .jpeg"
              class="form-control">
          </div>
          <div class="form-group">
            <input type="submit" name="saveEditedRecipe" id="saveEditedRecipe" value="Save Changes"
