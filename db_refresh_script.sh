...
mysql -u"$DB_USERNAME" --password="$DB_PASSWORD" -e "DROP DATABASE IF EXISTS $DB_DATABASE;"

echo "Création de la base de données MySQL..."
if ! mysql -u"$DB_USERNAME" --password="$DB_PASSWORD" -e "CREATE DATABASE IF NOT EXISTS $DB_DATABASE;" ; then
...
if [ -f db_sql/ntierprojet_database.sql ]; then
    ...
    mysql -u"$DB_USERNAME" --password="$DB_PASSWORD" "$DB_DATABASE" < db_sql/ntierprojet_database.sql
